...
namespace Ryn.CodeGen
{
    public class RynAstToCSharpVisitor
    {
        ...
        private StatementSyntax VisitStatement(AstNode node) => node switch
        {
            ...
            VarDeclNode decl => SyntaxFactory.LocalDeclarationStatement(
                SyntaxFactory.VariableDeclaration(
                    SyntaxFactory.IdentifierName("var"))
                .WithVariables(
                    SyntaxFactory.SingletonSeparatedList(
                        SyntaxFactory.VariableDeclarator(SyntaxFactory.Identifier(decl.Name))
                            .WithInitializer(
                                SyntaxFactory.EqualsValueClause(
                                    SyntaxFactory.ParseExpression(decl.Expression)))))),
